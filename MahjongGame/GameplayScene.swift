...
class GameplayScene: SKScene {
    ...
    init(size: CGSize, music: Bool, mark: Bool) {
      ...
      //  board.selectedBoard = boardType
        ...
        exitWindow!.position = CGPointMake(CGRectGetMidX(frame), CGRectGetMidY(frame))
        exitWindow!.zPosition = 600
        buttonFinish = TextAndButtons.createdButton()
        TextAndButtons.drawButton(buttonFinish!, text: "Finish", position: CGPointMake(0, -100), scene: exitWindow!)
    ...
    func configSelectorGlowing(selector: SKShapeNode) {
        
        selector.lineWidth = 3
        selector.glowWidth = 8.0
        
        //glowing animation
        let a = SKAction.fadeOutWithDuration(1)
        let b = SKAction.fadeInWithDuration(1)
        selector.runAction(SKAction.repeatActionForever(SKAction.sequence([a, b])))
    }
    ...
    func drawScreen() {
            ...
            for i in 0..<tilesInBoard![z].count {
                for j in 0..<tilesInBoard![z][i].count {
                    ...
                    if let ficha = tilesInBoard![z][i][j] {
                        ...
                        let newSize = CGSizeMake(ficha.figura!.size.width - 9, ficha.figura!.size.height - 9)
                        
                        drawSelect = SKShapeNode(rectOfSize: newSize, cornerRadius: 1)
                        hintSelect1 = SKShapeNode(rectOfSize: newSize, cornerRadius: 1)
                        hintSelect2 = SKShapeNode(rectOfSize: newSize, cornerRadius: 1)
                        drawSelect!.strokeColor = UIColor.greenColor()
                        hintSelect1!.strokeColor = UIColor.orangeColor()
                        hintSelect2!.strokeColor = UIColor.orangeColor()
                        ...
                        if ficha.special == specialTokens.TOKEN_NORMAL.hashValue ||
                            ...
                            let screenWidth = CGRectGetWidth(frame)
                            let screenHeight = CGRectGetHeight(frame) - buttonUndo.size.height
                            ...
                            if ficha.special == specialTokens.TOKEN_DOBLE_NORTH.hashValue {
                                ...
                                ficha.figura!.position = CGPointMake(coodX_1 + coodX_2, coodY_1 + coodY_2 + coodY_3 + coodY_4)
                                
                            } else if ficha.special == specialTokens.TOKEN_QUATUPLE_NORTH.hashValue {
                                
                                ficha.figura!.position = CGPointMake(coodX_1 + coodX_2 + coodX_3, coodY_1 + coodY_2 + coodY_3 + coodY_4)
                                
                            } else {
                                
                                ficha.figura!.position = CGPointMake(coodX_1 + coodX_2, coodY_1 + coodY_2 + coodY_3)
                            }
        ...
        if !gameOver {
            ...
            if currentState == gameStates.STATE_GAMEPLAY.hashValue {
                ...
                TextAndButtons.drawButton(buttonUndo, text:"Undo", position:CGPointMake(CGRectGetMinX(frame) + buttonUndo.size.width/2 + 10, CGRectGetMaxY(frame) - buttonUndo.size.height/2 - 15), scene:self)
                TextAndButtons.drawButton(buttonHint, text:"Hint", position:CGPointMake(buttonUndo.position.x + buttonUndo.size.width/2 + 40 + buttonHint.size.width/2, CGRectGetMaxY(frame) - buttonUndo.size.height/2 - 15), scene:self)
                TextAndButtons.drawButton(buttonPause, text:"Menu", position:CGPointMake(CGRectGetMaxX(frame) - buttonPause.size.width/2 - 10, CGRectGetMaxY(frame) - buttonPause.size.height/2 - 15), scene:self)
            }
            ...
            if exitWindow!.xScale == 0 {
                let animation1 = SKAction.scaleTo(1.3, duration: 0.5)
                let animation2 = SKAction.scaleTo(1, duration:0.2)
                exitWindow!.runAction(SKAction.sequence([animation1, animation2]))
            }
    ...
    func touchScreen(location: CGPoint, ended: Bool, touchMove: Bool) {
        ...
        if currentState == gameStates.STATE_GAMEPLAY.hashValue {
            if !gameOver {
                ...
                let newPoint = CGPointMake(location.x - size.width / 2.0, location.y - size.height / 2.0)
    ...
    func changeGreenColor(ficha: Ficha) {
        ficha.figura!.color = UIColor.greenColor()
        ficha.figura!.colorBlendFactor = 0.7
        ficha.selected = true;
        
        drawSelect!.position = CGPointMake(ficha.figura!.position.x - 4, ficha.figura!.position.y + 4)
    ...
    func changeOrangeColor(ficha: Ficha, fichaOne: Bool) {
        ficha.figura!.color = UIColor.orangeColor()
        ...
        if fichaOne {
            hintSelect1!.position = CGPointMake(ficha.figura!.position.x - 4, ficha.figura!.position.y + 4)
            ...
            hintSelect2!.position = CGPointMake(ficha.figura!.position.x - 4, ficha.figura!.position.y + 4)
...
//    }
    ...
    func normalColor(ficha: Ficha) {
        if boardSelected == boardSet.MARIO_BROS.hashValue {
            switch ficha.zPos {
            case 0:
                ficha.figura!.color = UIColor.yellowColor()
                break
            case 1:
                ficha.figura!.color = UIColor.grayColor()
                break
            case 2:
                ficha.figura!.color = UIColor.brownColor()
                break
            case 3:
                ficha.figura!.color = UIColor.redColor()
                break
            default:
                ficha.figura!.color = UIColor.whiteColor()
                break
            }
            ...
            case 0:
                ...
                break
            case 1:
                ficha.figura!.color = UIColor.blueColor()
                break
            case 2:
                ficha.figura!.color = UIColor.yellowColor()
                break
            case 3:
                ficha.figura!.color = UIColor.purpleColor()
                break
            case 4:
                ficha.figura!.color = UIColor.redColor()
                break
    ...
    func selectingToken(ficha: Ficha) {
    ...
    func deletedAnimation(token: Ficha) {
        let animation = SKAction.scaleTo(0, duration: 0.3)
        let deleted = SKAction.runBlock {
        ...
        token.figura?.runAction(SKAction.sequence([animation, deleted]))  //remove animation
        soundManager!.playSound("delete.m4a", scene: self)
    }
    
    
    
    func removeToken(token: Ficha, profundidad: Int, columna: Int, fila: Int) {
    ...
    func undoTokens() {
        ...
        if undoArray.count >= 2 {
            ...
            while i < 2 {
                ...
                getToken!!.figura!.runAction(SKAction.scaleTo(1.0, duration: 0.3))
    ...
    func scanBoard(consZ: Int, consI: Int, consJ: Int) {
    ...
    func showMessageWin(win: Bool) {
        
        if win {
            TextAndButtons.drawText("Congratulation!!", size: 30, position: CGPointMake(0, 100), scene: exitWindow!)
            TextAndButtons.drawText("You WIN!!", size: 30, position: CGPointMake(0, 0), scene: exitWindow!)
            soundManager!.playSound("win.m4a", scene: self)
            
        } else {
            TextAndButtons.drawText("No more moves", size: 30, position: CGPointMake(0, 100), scene: exitWindow!)
            TextAndButtons.drawText("Game Over :(", size: 30, position: CGPointMake(0, 0), scene: exitWindow!)
        }
